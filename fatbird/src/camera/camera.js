...
export class Camera
{
    ...
    update()
    {
        if (this.target)
        {
            ...
            this.position.x += (this.basePosition.x - this.position.x);
            this.position.y += (this.basePosition.y - this.position.y);

            // Clamp camera position to world boundaries if bounds are set
            if (this.bounds)
            {
                // Calculate the maximum allowed camera positions
                const minX = -this.bounds.width * this.zoom + this.app.screen.width;
                const minY = -this.bounds.height * this.zoom + this.app.screen.height;
                const maxX = 0;
                const maxY = 0;
